...
from app.models.adjustments import AdjustmentModel, AdjustmentUpdateModel, AdjustmentModelMixin
...
from app.db.base import Adjustment
...
def get_all_available_assets():
    return settings.AVAILABLE_ASSETS
...
def update_adjustment(adjustment_id: str, item: AdjustmentUpdateModel,
    ...
    user_id = user['id']
    payload = item.dict()
    # for delete_key in ['trend_values', 'buy_sell_percent']:
    #     if delete_key in payload:
    #         payload.pop(delete_key)
    ...
    if result.modified_count:
        try:
            ...
            background_tasks.add_task(LogsService, before_object=MongoService.convert_2_dict_mongodb(current_adj),
                                      ...
                                      action='update', fields=["name", 'assets'], user_id=user_id,
