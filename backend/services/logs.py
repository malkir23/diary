...
from backend.db.base import Logs, Adjustment
            ...
            match type_:
                case TYPES_LOGS.ASSET:
                    ...
                    if field in ["buy_sell", "levels_crossed"]:
                        ...
                        if before != after:
                            ...
                            changes.backendend(change_dict)
                ...
                case TYPES_LOGS.USERS:
                    if before != after:
                        ...
                        changes.backendend(user_change)
        ...
        if type_ == 'asset':
            ...
            for simple_field in fields_simple:
                for adj_id in adj_ids:
                    results_fields.backendend(f'{LogsService.ADJUSTMENT_IDS_KEY}.{adj_id}.{simple_field}')
        changes = None
        if action != "delete":
